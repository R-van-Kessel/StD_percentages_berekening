...
function generateQuestions() {
    ...
    // Genereer basis waardes voor een set van gerelateerde sommen
    for (let i = 0; i < 2; i++) {  // 2 sets van 5 variaties = 10 sommen
        // Basis waardes
        let origineel = floor(random(4, 20)) * 5;  // Bijvoorbeeld 25, 30, 35, etc
        let percentage = floor(random(3, 19)) * 5;  // Bijvoorbeeld 80, 75, 70, etc
        let decimaal = percentage / 100;
        let nieuw = round(origineel * decimaal);  // Bijvoorbeeld 20
        let percentageVerschil = 100 - percentage;  // Bijvoorbeeld 20
        let decimaalVerschil = percentageVerschil / 100;
        
        // Maak 5 verschillende variaties van dezelfde getallen
let variations = [
    // Variatie 1: Bereken nieuwe waarde na percentage korting
    // Vraag: 25€ - 20% = ?
    // Antwoord: 0,8 × 25
    {
        question: origineel + " - " + percentageVerschil + "% = ?",
        calculation: decimaal.toString().replace('.', ',') + " × " + origineel,
        answerId: 1
    },
    // Variatie 2: Bereken percentage van deel tot geheel
    // Vraag: 20€ is ?% van 25€
    // Antwoord: 20/25 × 100
    {
        question: nieuw + " is ?%\nvan " + origineel + "",
        calculation: nieuw + "/" + origineel + " × 100",
        answerId: 2
    },
    // Variatie 3: Bereken 100% als percentage bekend is
    // Vraag: 100% = ? als 20€ = 80%
    // Antwoord: 20/0,8
    {
        question: "100% = ?\nals " + nieuw + " = " + percentage + "%",
        calculation: nieuw + "/" + decimaal.toString().replace('.', ','),
        answerId: 3
    },
    // Variatie 4: Bereken nieuwe waarde na percentage verhoging
    // Vraag: 25€ + 20% = ?
    // Antwoord: 1,2 × 25
    {
        question: origineel + " + " + percentageVerschil + "% = ?",
        calculation: (1 + decimaalVerschil).toString().replace('.', ',') + " × " + origineel,
        answerId: 4
    },
    // Variatie 5: Bereken oorspronkelijk bedrag uit nieuw bedrag na korting
    // Vraag: ? - 20% = 20€
    // Antwoord: 20/0,8
    {
        question: "? - " + percentageVerschil + "%\n= " + nieuw + "",
        calculation: nieuw + "/" + decimaal.toString().replace('.', ','),
        answerId: 5
    }
];
        
        // Shuffle de variaties en voeg ze toe
        variations = shuffle(variations);
        for (let j = 0; j < 5; j++) {
            questions.push({
                text: variations[j].question,
                answer: variations[j].calculation,
                answerId: i * 10 + variations[j].answerId
            });
            answers.push({
                text: variations[j].calculation,
                answerId: i * 10 + variations[j].answerId
            });
        }
    }
    
    // Shuffle alleen de antwoorden
    ...
    for (let row = 0; row < 2; row++) {
        for (let col = 0; col < COLS; col++) {
            blocks.push({
                ...
                answerId: questions[questionIndex].answerId,
                isQuestion: true,
                isCorrect: null,
                isRatioTable: false,
                isPercentage: true
    ...
    for (let row = 2; row < 4; row++) {
        for (let col = 0; col < COLS; col++) {
            blocks.push({
                ...
                answer: answers[answerIndex].text,
                answerId: answers[answerIndex].answerId,
                isQuestion: false,
                isCorrect: null,
                isRatioTable: false,
                isPercentage: false
...
function drawBlock(block) {
    ...
    // Bepaal tekstgrootte op basis van type blok
    if (block.isRatioTable) {
        textSize(12); // Kleiner lettertype voor verhoudingstabellen
    } else if (block.isPercentage) {
        textSize(12); // Nog kleiner voor percentage sommen
    } else {
        textSize(13); // Normaal lettertype voor antwoorden
    }
    ...
    if (block.text.includes('\n')) {
        ...
        if (block.isRatioTable) {
            // Voor verhoudingstabellen: splits per regel en per kolom
            let row1Parts = lines[0].split(' '); 
            let row2Parts = lines[1].split(' ');
            
            let quarterX = CELL_SIZE / 4;
            let quarterY = CELL_SIZE / 4;
            
            // Teken waarden in de 4 vakjes
            text(row1Parts[0], block.x + quarterX, block.y + quarterY);
            text(row1Parts[1], block.x + 3 * quarterX, block.y + quarterY);
            text(row2Parts[0], block.x + quarterX, block.y + 3 * quarterY);
            text(row2Parts[1], block.x + 3 * quarterX, block.y + 3 * quarterY);
        } else {
            // Voor normale multi-line tekst
            let lineHeight = 16;
        ...
        // Tekst met breuken
        drawTextWithFractions(block.text, block.x, block.y, CELL_SIZE);
    } else if (block.isPercentage) {
        // Automatische terugloop voor percentage sommen zonder breuken
        let words = block.text.split(' ');
        let lines = [];
        let currentLine = '';
        let maxWidth = CELL_SIZE - 20;
        ...
        // Normale enkele regel tekst
        text(block.text, block.x + CELL_SIZE / 2, block.y + CELL_SIZE / 2);
    }
}

// Hulpfunctie om tekst met breuken te tekenen
function drawTextWithFractions(txt, blockX, blockY, cellSize) {
    let parts = txt.split(' ');
    let centerY = blockY + cellSize / 2;
    
    // Bereken totale breedte eerst
    textSize(12);
    let totalWidth = 0;
    for (let part of parts) {
        if (part.includes('/')) {
            totalWidth += 25; // Breedte van breuk
        } else {
            totalWidth += textWidth(part);
        }
        totalWidth += textWidth(' '); // Spatie tussen delen
    }
    
    // Start positie (gecentreerd)
    let startX = blockX + cellSize/2 - totalWidth/2;
    ...
    for (let part of parts) {
        if (part.includes('/')) {
            ...
            textSize(12);  //breuk grootte
            text(num, startX, centerY - 8);  // Teller
            
            stroke(255);
            strokeWeight(2);
            line(startX - 10, centerY, startX + 10, centerY);  // Breukstreep
            noStroke();
            
            text(den, startX, centerY + 8);  // Noemer
            ...
            startX += textWidth(part) + textWidth(' ');
        }
    }
}
function checkAnswers() {
    ...
    for (let block of blocks) {
        if (block.isQuestion) {
            ...
            for (let other of blocks) {
                if (!other.isQuestion && other.col === block.col && other.row === block.row + 2) {
                    answerBlock = other;
                    break;
                }
            }
            
            if (answerBlock && answerBlock.answerId === block.answerId) {
    ...
    if (scoreButton && scoreButton.elt) {
        scoreButton.elt.textContent = 'Score: ' + correctCount + '/10';
        if (correctCount === 10) {
            scoreButton.style('background-color', '#4CAF50');
        } else {
            scoreButton.style('background-color', '#FF5722');
        }
    }
}
